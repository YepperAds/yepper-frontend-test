  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (authMode === 'login') {
        ...
        // Pass email directly — user state won't have updated yet (React state is async)
        await createAdAndProceed(authData.email);
      } else {
              ...
              {!isAuthenticated ? (
                <div className="space-y-6">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center px-4 py-3.5 border-2 border-gray-200 bg-white hover:bg-gray-50 hover:border-gray-300 transition-all duration-200 font-semibold text-gray-700 text-base shadow-sm disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {isGoogleLoading ? (
                  ...
                  <div className="relative flex items-center">
                    ...
                    <span className="px-3 text-xs text-gray-400 bg-white">or sign in with email</span>
                    <div className="flex-1 border-t border-gray-200" />
                  </div>
